...
export default {
  ...
  data() {
    return {
      isLoading: false,
      selectedPlace: 'Fraser Valley',
      month: new Date().getMonth() + 1,
      year: new Date().getFullYear(),
      data: null,
      selectedCity: 'fraser-valley',
    };
  },
  computed: {
    lastAvailable: {
    ...
    async fetchData() {
      try {
        ...
        this.data = await this.$axios.$get(
          `/reports/data/fraser-valley/${this.year}/${this.month}/${this.selectedPlace}`,
        );
    ...
    async putData() {
      try {
        ...
        await this.$axios.$put(
          `/reports/data/fraser-valley/${this.year}/${this.month}/${encodeURI(
            this.selectedPlace,
          )}`,
          this.data,
        );
